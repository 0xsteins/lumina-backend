...
export async function getTransactions(pool: Pool, limit: number, cursor?: string | null) {
  const { rows } = await pool.query<TransactionRow>(
    `SELECT * FROM transactions
     WHERE $2::text IS NULL OR (ledger, hash) < (SELECT ledger, hash FROM transactions WHERE hash = $2)
...
export async function getTransactionByHash(pool: Pool, hash: string) {
  const { rows } = await pool.query<TransactionRow>('SELECT * FROM transactions WHERE hash = $1', [hash]);
...
export async function getOperations(
  ...
  if (opts.cursor) {
    ...
    conditions.push(`(ledger, id) < (SELECT ledger, id FROM operations WHERE id = $${params.length})`);
  }
...
export async function getOperationsByTransactionHash(pool: Pool, hash: string) {
  const { rows } = await pool.query<OperationRow>(
    'SELECT * FROM operations WHERE transaction_hash = $1 ORDER BY id ASC',
    [hash]
  );
  return rows.map(mapOperation);
}
...
export async function getLatestLedgerFromDb(pool: Pool) {
  const { rows } = await pool.query<LedgerRow>('SELECT * FROM ledgers ORDER BY sequence DESC LIMIT 1');
  return rows[0] ? mapLedger(rows[0]) : null;
}

export async function getAccountFromDb(pool: Pool, address: string) {
  const { rows } = await pool.query<AccountRow>('SELECT * FROM accounts WHERE address = $1', [address]);
...
export async function getAccountTransactions(pool: Pool, address: string, limit: number) {
  const { rows } = await pool.query<TransactionRow>(
    'SELECT * FROM transactions WHERE source_account = $1 ORDER BY ledger DESC LIMIT $2',
    [address, limit]
  );
  return rows.map(mapTransaction);
}

export async function getAccountOperations(pool: Pool, address: string, limit: number) {
  const { rows } = await pool.query<OperationRow>(
    'SELECT * FROM operations WHERE source_account = $1 ORDER BY ledger DESC LIMIT $2',
    [address, limit]
  );
  return rows.map(mapOperation);
}
...
export async function getEventsByContract(
  pool: Pool,
  opts: { contractId: string; topic?: string | null; limit: number; cursor?: string | null }
) {
  ...
  if (opts.cursor) {
    params.push(opts.cursor);
    conditions.push(`(ledger, id) < (SELECT ledger, id FROM contract_events WHERE id = $${params.length})`);
  }
